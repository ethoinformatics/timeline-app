...
// Components
var PopupButtons = require('popup-buttons');
var CreateSelectMenu = require('../../../create-select-dialog');
var EditExistingForm = require('edit-existing-form');

// Templates
var inlineChildTmpl = require('./inline-child.vash');
var scrollTmpl = require('./scroll.vash');
...
function EditTab(opts, listViewReference) {
  ...
  // Render template and create DOM element
  var htmlFromTemplate = scrollTmpl({});
  self.$element = $(htmlFromTemplate);
  ...
  // TODO: REFACTOR >> This kind of code is so fragile.   
  ...
  function _collapseChildren(collectionName) {
    ...
    function _addInlineChild(collectionName, domainName) {
      ...
      // ??? Why is this event handler nested?
  ...
  self.saveAndReloadDiary = function() {
    ...
    var rootDomain = app.getDomain(rootEntity.domainName);
    var rootEntityManager = rootDomain.getService('entity-manager');
  ...
  // Bind to taps on the triangle indicator to show / hide items
  // TODO: REFACTOR
  self.addTriangleHandlers = function() {
    self.$element.find('.item.item-divider.header').each(function( index ) {
      ...
      // Bind touch and mouse clicks
      element.on('touchstart', function(e) {
        // Toggle nextAll() elements
        // This is a list, so next items are <li>
        element.nextAll().toggle();

        // Add or remove triangle indicator depending on whether
        // list is expanded
        if ( element.next().is(':visible') ) {
          element.find('.tri0').addClass('toggleTriangle');
        }else{
          element.find('.tri0').removeClass('toggleTriangle');
        }
  ...
  /*self.toggleAccordian = function() {
    console.log("ViewExistingDialog > Tabs > Edit >>> toggleAccordian()");
    $( this ).nextAll().toggle();
    if( $( this ).next().is(":visible") ) {
      $( this ).find('.tri0').addClass('toggleTriangle');
    }else{
